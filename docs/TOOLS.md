...
Разбить ОДНУ цель на ОДНУ задачу + чеклист шагов (план). Шаги становятся пунктами чеклиста, а не отдельными задачами. Для нескольких разных дел вызывай по разу на каждое (или create_task на каждое).

- **Вход:**
  - `goal`: string **(обяз.)** — ОДНА цель одной фразой (станет заголовком задачи)
  - `steps`: array — Шаги-пункты чеклиста. Если пусто — типовой шаблон.
  - `project`: string | null
  - `priority`: enum/ref
  - `deadline`: string | null
- **Выход:** {task: Task, checklist: Checklist}
